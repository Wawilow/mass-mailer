...
if not sys.version_info[0] > 2 and not sys.version_info[1] > 8:
    exit(
        '\033[0;31mpython 3.9 is required. try to run this script with \033[1mpython3\033[0;31m instead of \033[1mpython\033[0m')

# dangerous mx domains, skipping them all
dangerous_domains = r'\d'
...
def show_banner():
    banner = f"""
                                         ...
                                         `8P  88P                        
                                             d88                         
 ?88   d8P  d8P d888b8b   ?88   d8P  d8P  88b888   d8888b  ?88   d8P  d8P
 d88  d8P' d8P'd8P' ?88   d88  d8P' d8P'  88P?88  d8P' ?88 d88  d8P' d8P'
 ?8b ,88b ,88' 88b  ,88b  ?8b ,88b ,88'  d88  88b 88b  d88 ?8b ,88b ,88' 
 `?888P'888P'  `?88P'`88b `?888P'888P'  d88'   88b`?8888P' `?888P'888P'  
 
        {b}Original github repo: https://github.com/aels/mailtools{b}
        Made by {b}Aels{b}, Edited by {b}George{b}
        https://github.com/wawilow/mass-mailer
        My email: george@lame.solutions, My website: lame.solutions, My telegram: t.me/wawilow
        Feel fre to contact me for future work!"""
...
def check_ipv4():
    ...
    try:
        ...
        socket.ipv4_blacklist = re.findall(r'"name":"([^"]+)","listed":true',
                                           read('https://addon.dnslytics.net/ipv4info/v1/' + socket.has_ipv4))
...
def read(path):
    return os.path.isfile(path) and open(path, 'r', encoding='utf-8-sig', errors='ignore').read() or re.search(
        r'^https?://', path) and requests.get(path, timeout=5).text or ''
...
def is_dangerous_email(email):
    ...
    try:
        ...
        return mx_domain if re.findall(dangerous_domains, mx_domain) and not re.findall(r'\.outlook\.com$',
                                                                                        mx_domain) else False
    except:
...
def create_attachment(file_path, subs):
    ...
    if is_file_or_url(file_path):
        ...
        attachment_body = expand_macros(read(file_path), subs) if attachment_ext in text_extensions else read_bytes(
            file_path)
...
def smtp_sendmail(mail_str):
    ...
    mail_body = expand_macros(read(config['mail_body']) if is_file_or_url(config['mail_body']) else config['mail_body'],
                              subs)
                ...
                while True:
                    ...
                    try:
                        ...
                        if is_dangerous := is_dangerous_email(mail_to):
                            msg = red('-\b' + '>' * (mails_sent % 3) + b + '>', 2) + red('>> '[mails_sent % 3:],
                                                                                         2) + 'skipping email - ' + mail_to + ' (' + red(
                                is_dangerous) + ')'
                        ...
                        if re.search(
                                r'suspicio|suspended|too many|limit|spam|blocked|unexpectedly closed|mailbox unavailable',
                                str(e).lower()):
                            raise Exception(e)
...
def load_config():
    ...
    if len(sys.argv) == 2:
        config['config_file'] = sys.argv[1] if is_file_or_url(sys.argv[1]) else exit(
            err + 'wrong config path or filename: it must be like ' + bold('<...>.config'))
    else:
        try:
            ...
            exit(wrn + 'sample ' + bold(
                'dummy.config') + ' file downloaded to the current directory. please check and edit it before next run')
    ...
    if not is_file_or_url(config['smtps_list_file']):
        ...
        for smtp_line in smtp_pool_array:
            ...
            not re.findall(r'^[\w.+-]+\|\d+\|[@\w.+-]+\|[^|]+$', smtp_line) and exit(
                err + '"' + smtp_line + '" is not like "host|port|username|password"')
    try:
    ...
    if len([is_valid_email(mail) for mail in config['mails_to_verify'].split(',')]) < config['mails_to_verify'].count(
            ',') + 1:
    ...
    config['mail_subject'] or exit(
        err + 'please fulfill ' + bold('mail_subject') + ' parameter with desired email subject')
    config['mail_body'] or exit(
        err + 'please put the path to email body file or mail body itself as a string into ' + bold(
            'mail_body') + ' parameter')
...
def logs_writer():
    ...
    with open(config['smtps_errors_file'], 'a') as smtps_errors_file_handle, open(config['mails_dangerous_file'],
                                                                                  'a') as mails_dangerous_file_handle:
        while True:
            while not smtp_errors_que.empty():
                smtp_str, msg, smtp_sent = smtp_errors_que.get()
                smtps_errors_file_handle.write(
                    now() + ' ' + smtp_str + ' (' + str(smtp_sent) + ' emails): ' + msg.split('\b')[-1] + '\n')
